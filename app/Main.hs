...
main = do
  ...
  -- duckdbQuery res "DESCRIBE read_parquet('/Users/rahulsingh/Downloads/t.parquet');"
  ...
  startTime <- nominalDiffTimeToSeconds <$> getPOSIXTime
  runConduit $ do
            (duckdbQueryWithResponse res "SELECT  ROUND( (SUM(sign * IF(is_success = 1, 1, 0)) * 100.0) / NULLIF(SUM(sign * 1), 0), 2 ) AS success_rate, SUM(sign * 1) AS total_volume, card_brand, payment_method_type, payment_gateway FROM read_parquet('/Users/rahulsingh/Downloads/t.parquet') WHERE card_brand IN ( SELECT card_brand FROM read_parquet('/Users/rahulsingh/Downloads/t.parquet') WHERE merchant_id = 'floweraura' GROUP BY card_brand ORDER BY SUM(sign * 1) DESC LIMIT 5 ) AND payment_method_type IN ( SELECT payment_method_type FROM read_parquet('/Users/rahulsingh/Downloads/t.parquet') WHERE merchant_id = 'floweraura' GROUP BY payment_method_type ORDER BY SUM(sign * 1) DESC LIMIT 5 ) AND payment_gateway IN ( SELECT payment_gateway FROM read_parquet('/Users/rahulsingh/Downloads/t.parquet') WHERE merchant_id = 'floweraura' GROUP BY payment_gateway ORDER BY SUM(sign * 1) DESC LIMIT 5 ) AND merchant_id = 'floweraura'  GROUP BY card_brand, payment_method_type, payment_gateway having success_rate IS NOT NULL  ORDER BY (SUM(sign * 1)) DESC;")
            .| Conduit.map (encode)
            .| Conduit.map (BS.toStrict)
            .| Conduit.map (<> "\n")
            .| Conduit.stdout
  endTime <- nominalDiffTimeToSeconds <$> getPOSIXTime
  print ("time taken:" <> (show $ endTime - startTime))
  print "testing"
  duckdbClose res
  pure ()
...
-- SELECT ROUND( (SUM(sign * IF(is_success = 1, 1, 0)) * 100.0) / NULLIF(SUM(sign * 1), 0), 2 ) AS success_rate, SUM(sign * 1) AS total_volume, card_brand, payment_method_type, payment_gateway FROM read_parquet('/Users/rahulsingh/Downloads/t.parquet') WHERE card_brand IN ( SELECT card_brand FROM read_parquet('/Users/rahulsingh/Downloads/t.parquet') WHERE merchant_id = 'floweraura' GROUP BY card_brand ORDER BY SUM(sign * 1) DESC LIMIT 5 ) group by card_brand, payment_method_type, payment_gateway